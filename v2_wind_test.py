...
def run_burst_vid2(
    ...
    print(f"Test 3: Burst (libcamera-vid) @ {fps} fps")

    video_file = f"03-burst_{fps}fps.mjpeg"
    frame_pattern = f"03-frame_{fps}fps_%04d.jpg"

    # --- limpiar frames anteriores ---
    for f in os.listdir("."):
        if f.startswith(f"03-frame_{fps}fps_"):
            os.remove(f)
    ...
    for idx, fname in enumerate(frames):
        writer.writerow({
            ...
            "sensor_temperature": None,
            "exposure_time_reported": shutter_us,
            "notes": f"libcamera-vid burst @ {fps} fps"
        })
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.8,
        help="Factor del shutter automático para el burst (default 0.8)"
    )
    ...
    with open("metadata.csv", "w", newline="") as csvfile:
        ...
        print("Test 3: Temporal sampling (FPS sweep)")

        burst_shutter = shutter_auto * args.burst_factor

        for fps in BURST_FPS_LIST:
            run_burst_vid2(
                shutter_us=burst_shutter,
                gain=gain_auto,
                burst_factor=args.burst_factor,
